...
function buildDockerImage() {
    ...
    // const baseDockerImage = "public.ecr.aws/lambda/python:3.9"
    ...
    const dockerFileContents =
    `
    FROM public.ecr.aws/shogo82148/lambda-python:build-3.9 AS builder
    USER root
    ...
    FROM public.ecr.aws/shogo82148/lambda-python:3.9
    COPY --from=builder /tmp /var/task/${this.serverless.service.service}/
    RUN rm -rf /tmp/*
    `;
    ...
    const dockerOptions = ['build', '-f', dockerFile, '-t', this.provider.naming.getDockerImageName(), '--platform', 'linux/amd64', '.'];
